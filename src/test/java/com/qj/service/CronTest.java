...
//        Duration timeFromLastExecution = executionTime.timeToNextExecution(now);
...
//        );
        ...
        CronParser qp = new CronParser(CronDefinitionBuilder.instanceDefinitionFor(UNIX));
        Cron c = qp.parse("*/2 * * * *");
        ExecutionTime et = ExecutionTime.forCron(c);
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").withZone(ZoneId.of("Asia/Shanghai"));
        ZonedDateTime time = ZonedDateTime.parse("2018-07-12 13:59:59", formatter);
        Optional<ZonedDateTime> next = et.nextExecution(time);
        long i = next.get().toEpochSecond() - time.toEpochSecond();
        System.out.println(i + "###" + DateUtil.time2str(next.get().toEpochSecond()));
    }
